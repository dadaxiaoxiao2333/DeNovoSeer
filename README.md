...
The complete preprocessing workflow is provided in this notebook, enabling users to regenerate the final processed datasets from the input data and apply the framework to independent datasets.
...
**Model checkpoints:**
The pre‑trained models in `best_model/` were produced by exactly this script. 

**Pre-trained model usage:**

Representative pretrained checkpoints are provided in the `best_model/` folder:

- `best_model_seed42.pth` – representative model trained with random seed 42

These checkpoints can be directly loaded for prediction and interpretation without retraining.
...
checkpoint = torch.load(
    "best_model/best_model_seed42.pth",
    map_location="cpu"
)
...
You can also train from scratch and your results will be saved in the working directory (you may then move them to replace the existing ones).
